...
| Need | Tool |
|------|------|
| Authenticated browsing (existing logins, confidential sites) | `browser-use --browser real` |
| Sandboxed automation (no credentials needed) | `browser-use --browser chromium` (default) |
| JS debugging, network inspection, console errors | `chrome-devtools` MCP |

Use `--browser real` whenever the site requires login — it attaches to your running Chrome with full cookie/session state. The shell alias defaults to real mode + work profile so `browser-use open <url>` just works for authenticated sites.

## Browser Modes

```bash
browser-use open <url>                                        # Default alias: real + work profile
browser-use --browser real --profile "Profile 1" open <url>  # Work profile (ted@heidihealth.com)
browser-use --browser real --profile "Profile 2" open <url>  # Personal (ted.zhao.au@gmail.com)
browser-use --browser chromium open <url>                     # Isolated headless (no login state)
...
- **real**: Attaches to running Chrome — uses your cookies, extensions, logged-in sessions
- **chromium**: Fresh isolated browser, fast, no state
- **remote**: Cloud-hosted browser with proxy support (requires BROWSER_USE_API_KEY)

**Chrome profiles (real mode):**
| Profile | Account | Use for |
|---------|---------|---------|
| `Profile 1` | ted@heidihealth.com | Work (default alias) |
| `Profile 2` | ted.zhao.au@gmail.com | Personal |
| `Default` | zp913913@gmail.com | — |
...
```bash
# Default alias already uses real + work profile
browser-use open https://app.heidihealth.com
browser-use state  # Already logged in as ted@heidihealth.com

# Explicitly specify work profile
browser-use --browser real --profile "Profile 1" open https://internal-tool.com
```
